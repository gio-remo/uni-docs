...
Many cells (6,7 mln) have value 0. This would imply that natural birth and death rate perfectly match. Leaving these 0 in the dataset alters the analysis, and as a perfect match of the demographic rates seems rather a rare residual case.
...
2. Both datasets have the same resolution: 0.83° / 5 arc-min https://www.inchcalculator.com/convert/degree-to-arcminute/

```{r}
# Net-Migration dataset uses WGS 84 (EPSG:4326) 
raster_netMgr_2000_2019_annual
```
...
# Income dataset also uses WGS84
...
Let's overlap them to check whether they're actually using the same grid system!
...
Spatial join (https://tmieno2.github.io/R-as-GIS-for-Economists-Quarto/chapters/03-SpatialInteractionVectorVector.html#spatial-join-with-sfst_join)

```{r}
library(sf)

# Convert both dataframes to sf objects
sf_gdp <- st_as_sf(gdp, coords = c("lon", "lat"), crs = 4326)
sf_netmgr <- st_as_sf(df_netMgr_2000_2019_annual_long, coords = c("lon", "lat"), crs = 4326)

# Match nearest income point to each netmgr point
nearest_id <- st_nearest_feature(sf_netmgr, sf_gdp)

# Extract matched quartiles
df_netMgr_2000_2019_annual_long$gdp_quartile <- gdp$gdp_quartile[nearest_id]
```
