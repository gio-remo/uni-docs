...
income_urban <- read.csv("urban-income.csv")
...
```{r}
temperature <- read.csv("temperature_summary.csv")

# Extract coordinates
coords1 <- as.matrix(df_merged[, c("x", "y")])
coords2 <- as.matrix(temperature[, c("x", "y")])
...
ggplot(df_merged_temp) +
  geom_raster(aes(x = x, y = y, fill = avg_temp)) +
  coord_fixed() +
...
```{r}
precipitation <- read.csv("total_precipitation_summary.csv")

# Extract coordinates
coords1 <- as.matrix(df_merged_temp[, c("x", "y")])
coords2 <- as.matrix(precipitation[, c("x", "y")])
...
ggplot(df_merged_final) +
  geom_raster(aes(x = x, y = y, fill = avg_tp)) +
  coord_fixed() +
  scale_fill_viridis_c()
```
...
df_netMgr_2000_2019_annual_long <- df_merged_final %>%
  pivot_longer(
    cols = all_of(years),      # only pivot year columns
    names_to = "year",
    values_to = "netmgr"
  ) # 28 mln obs
...
```{r}
df_netMgr_2000_2019_annual_long[df_netMgr_2000_2019_annual_long$netmgr == 0, ] # 2,7 mln obs
```

```{r}
zeros_by_location <- df_netMgr_2000_2019_annual_long %>%
  filter(year == 2000 & netmgr == 0) # 130k obs
...
```{r}
# Remove 0s
df_netMgr_2000_2019_annual_long <- df_netMgr_2000_2019_annual_long[df_netMgr_2000_2019_annual_long$netmgr != 0,] # 25mln obs
...
```{r}
p <- quantile(df_netMgr_2000_2019_annual_long$netmgr, probs = c(0.005, 0.995))
...
df_netMgr_2000_2019_annual_long <- df_netMgr_2000_2019_annual_long[
  ...
  df_netMgr_2000_2019_annual_long$netmgr <= p[2], ] # From 25,2mln to 24,9mln obs
...
netmgr_1y <- df_netMgr_2000_2019_annual_long[df_netMgr_2000_2019_annual_long$year == 2000,] # 1,2mln obs
...
p2 <- ggplot(netmgr_1y) + 
  geom_raster(aes(x = x, y = y, fill = netmgr)) +
  coord_fixed() +
...
p3 <- ggplot() +
  geom_tile(data = netmgr_subset, aes(x = x, y = y, fill = mean_gdp)) +
  coord_fixed() +
...
```{r}
one_year <- df_netMgr_2000_2019_annual_long[df_netMgr_2000_2019_annual_long$year == 2000, ]

# For full sample: make Group character explicitly
summary_temp_full_sample <- summary_stats(one_year, avg_temp) %>%
  mutate(Group = "Full sample")

# For group summaries: convert Group to character after rename
summary_temp_by_group <- one_year %>%
  group_by(gdp_quartile) %>%
...
```{r}
# For full sample: make Group character explicitly
summary_prec_full_sample <- summary_stats(one_year, avg_tp) %>%
  mutate(Group = "Full sample")

# For group summaries: convert Group to character after rename
summary_prec_by_group <- one_year %>%
  group_by(gdp_quartile) %>%
...
gt(summary_table_prec) %>% 
  ...
  fmt_number(columns = c(3,4,5,6), decimals = 2) %>% 
  tab_options(table.width = pct(80)) %>% 
  tab_header(title = "Total Precipitation") %>% 
