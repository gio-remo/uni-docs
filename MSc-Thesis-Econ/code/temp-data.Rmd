...
### Merge Temp to Income Quartile

1) RESHAPING TEMP TO LONG

```{r}
library(data.table)

# Assign year names to the layers
names(yearly_rasters) <- 2000:2019

# Get coordinates of all raster cells
coords <- crds(yearly_rasters, na.rm = FALSE)  # matrix [n_cells, 2], columns: x, y

# Get temperature values for each cell and year
temp_vals <- as.matrix(yearly_rasters)  # matrix [n_cells, 20]
...
dt_temp <- as.data.table(temp_vals)
dt <- cbind(dt, dt_temp)

# To long format: one row per cell-year
dt_long <- melt(
  dt,
  id.vars = c("x", "y"),
  variable.name = "year",
...
# Remove NAs
dt_long <- dt_long[!is.na(dt_long$temp),]
```
...
gdp <- read.csv("income-gdp-quarter.csv")

# Extract coordinates for temp
temp_coords <- as.matrix(dt_long[, .(x, y)])

# Extract coordinates for gdp
gdp_coords <- as.matrix(gdp[, c("x", "y")])

# Find index of nearest
nn_idx <- get.knnx(gdp_coords, temp_coords, k = 1)$nn.index[, 1]

# Merge
dt_long[, income_q := gdp$gdp_quartile[nn_idx]]
```
...
summary_temp_full_sample <- summary_stats(dt_long, temp) %>%
  mutate(Group = "Full sample")

# Summary by GDP group
summary_temp_by_group <- dt_long %>%
  group_by(income_q) %>%
  # Apply function to each group
  group_modify(~summary_stats(.x, temp)) %>%
  rename(Group = income_q)
